...
export default class ViewerImages extends Viewer {
	constructor({
		...
		if (!ppixiv.mobile) {
			this._imageEditor = new ImageEditor({
				...
				onVisibilityChanged: () => {
					this.refresh();
				}, // refresh when crop editing is changed
			});
		}
	}
	...
	async load() {
		...
		if (this._imageEditor) this._imageEditor.setMediaId(this.mediaId);
	...
	_mediaInfoModified({ mediaId }) {
		if (mediaId != this.mediaId) return;
	...
	_refreshFromMediaInfo() {
		...
		if (crop && !haveActualResolution) return;

		// Update any custom pan created by PanEditor.
		this._custom_animation = pan;

		// Disable cropping if the crop editor is active.
		if (this._imageEditor?.editingCrop) crop = null;

		let croppedSize =
			crop && crop.length == 4
				? new FixedDOMRect(crop[0], crop[1], crop[2], crop[3])
				: null;
	...
	async _refreshImage({ url, previewUrl, translationUrl, inpaintUrl, signal }) {
		...
		if (this._pointerListener) this._pointerListener.checkMissedClicks();

		// Don't show low-res previews during slideshows.
		if (this._slideshowMode) previewUrl = url;
		...
		// refreshing.
		if (this._imageContainer.displayedImage == "main") return;
		...
		if (!ppixiv.mobile) {
			if (url != null && this._imageContainer.complete) {
				...
				imageReady =
					(await helpers.other.awaitWithTimeout(decodePromise, 50)) !=
					"timed-out";
			}
			signal.throwIfAborted();
		}
		...
		if (this._slideshowMode) this.pauseAnimation = true;
		...
		// to finish.
		if (!decodePromise) decodePromise = this._decodeImage(this._imageContainer);
		await decodePromise;
		signal.throwIfAborted();
	...
	_onresize = (e) => {
		...
		if (this._imageContainer.displayedImage == null) return;

		this._setImageBoxSize();
		this._reposition();

		// If the window size changes while we have an animation running, update the animation.
		if (this._animationsRunning) this._refreshAnimation();
	};
	...
	setZoom({ enabled = null, level = null, stopAnimation = true } = {}) {
		if (stopAnimation) this._stopAnimation();

		if (enabled != null) {
			// Zoom lock is always enabled on mobile.
			if (ppixiv.mobile) enabled = true;
	...
	get _zoomLevelEffective() {
		if (!this.zoomActive) return 0;
		else return this._zoomLevelCurrent;
	}
	...
	get _zoomLevelCurrent() {
		...
		if (level == "cover") return this._zoomLevelCover;
		else if (level == "actual") return this._zoomLevelActual;
		else return level;
	}
	...
	get _zoomFactorCover() {
		let result =
			Math.max(this.viewWidth / this.width, this.viewHeight / this.height) || 1;
	...
	get _zoomFactorContain() {
		let result =
			Math.min(this.viewWidth / this.width, this.viewHeight / this.height) || 1;
	...
	get _zoomFactorActual() {
		if (this._actualWidth == null) return 1;
		else return this._actualWidth / this.width;
	}
	...
	changeZoom(zoomOut, { stopAnimation = true } = {}) {
		...
		else newLevel = Math.ceil((newLevel + 0.001) / increment) * increment;
		...
		if (crossed(oldLevel, newLevel, this._zoomLevelCover)) {
			...
			this._actualZoomAvailable &&
			crossed(oldLevel, newLevel, this._zoomLevelActual)
		) {
	...
	getImagePosition(viewPos, { pos = null } = {}) {
		if (pos == null) pos = this._currentZoomPos;
	...
	getCenterForImagePosition(viewPos, zoomCenter) {
		return [
			...
			-(
				(viewPos[1] - this.viewHeight / 2) / this.currentHeight -
				zoomCenter[1]
			),
		];
	}
	...
	get _currentZoomPos() {
		if (this.zoomActive) return [this._centerPos[0], this._centerPos[1]];
		else return [0.5, 0.5];
	}
	...
	_reposition({ clampPosition = true } = {}) {
		if (this._imageContainer == null || !this._initialPositionSet) return;
	...
	getCurrentActualPosition({
		...
		if (zoomPos == null) zoomPos = this._currentZoomPos;
		...
		if (clampPosition) {
			...
			if (viewWidth >= Math.floor(zoomedWidth)) zoomPos[0] = 0.5; // center horizontally
			if (viewHeight >= Math.floor(zoomedHeight)) zoomPos[1] = 0.5; // center vertically
		}
		...
		// smoother panning.
		let inActualZoomMode =
			this._actualZoomAvailable &&
			Math.abs(this._zoomFactorEffective - this._zoomFactorActual) < 0.001;
		if (inActualZoomMode) {
			x = Math.round(x);
			y = Math.round(y);
		}
	...
	_setInitialImagePosition(restoreHistory) {
		...
		if (args.state.zoom?.animating) this._refreshAnimation();
	...
	_scheduleSaveToHistory() {
		...
		if (this._saveToHistoryId) return;
	...
	_createCurrentAnimation() {
		...
		let animationMode;
		if (this._slideshowMode == "loop") animationMode = "loop";
		else if (this._slideshowMode != null) animationMode = "slideshow";
		else if (ppixiv.settings.get("auto_pan")) animationMode = "auto-pan";
		else return {};
		...
		// while dragging.
		if (this.options.displayedByDrag) animation.fadeIn = 0;
	...
	_refreshAnimation() {
		...
		if (animationMode == "auto-pan") {
			if (this._ranPanAnimation) return;
	...
	_checkAnimationFinished = async (e) => {
		if (this._animations.main?.playState != "finished") return;
	...
	_stopAnimation({
		...
		if (!this._animations.main) return false;
		...
		try {
			for (let [name, animation] of Object.entries(this._animations)) {
				if (keepAnimations.indexOf(name) != -1) continue;
	...
	zoomAdjust(down, { x, y }) {
		...
		if (!this.zoomActive) this.setZoom({ enabled: true, level: 0 });
		...
		if (Math.abs(oldZoomLevel - newZoomLevel) < 0.01) this.changeZoom(down);
...
class ImagesContainer extends Widget {
	...
	get complete() {
		return (
			...
			this.inpaintImage.complete &&
			this.translationImage.complete
		);
	}

	decode() {
		let promises = [];
		if (this.mainImage.src) promises.push(this.mainImage.decode());
		if (this.inpaintImage.src) promises.push(this.inpaintImage.decode());
	...
	get displayedImage() {
		if (!this.mainImage.hidden) return "main";
		else if (!this.previewImage.hidden) return "preview";
		else return null;
	}
	...
	_refreshInpaintVisibility() {
		...
		this.translationImage.hidden =
			this.mainImage.hidden || !this.translationImage.src;
	}
