...
export default class ImageEditor extends IllustWidget {
	constructor({
		...
		if (visible == null) visible = ppixiv.settings.get("image_editing", false);
		...
		this._showInpaint.addEventListener("click", (e) => {
			...
			this.activeEditorName =
				this.activeEditorName == "inpaint" ? null : "inpaint";
		});
	...
	get editingCrop() {
		return (
			ppixiv.settings.get("image_editing", false) &&
			this.activeEditorName == "crop"
		);
	}
	...
	refresh() {
		...
		if (this._mediaId != null) this._showInpaint.hidden = !isLocal;
		...
		let showingInpaint =
			isLocal && this.activeEditorName == "inpaint" && this.visible;
		...
		this._topButtonRow.querySelector(".left").hidden =
		...
		else HideMouseCursorOnIdle.enableAll("image-editing");
	}
	...
	getDataToSave({ includeEmpty = true } = {}) {
		...
		for (let editor of Object.values(this.editors)) {
			for (let [key, value] of Object.entries(editor.getDataToSave())) {
				if (includeEmpty || value != null) edits[key] = value;
			}
		}
		return edits;
	}
	...
	async save() {
		...
		try {
			...
			if (currentEditor?.afterSave) currentEditor.afterSave(mediaInfo);
		} finally {
	...
	get activeEditor() {
		...
		else return this.editors[currentEditor];
	}
