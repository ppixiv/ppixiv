...
export default class DesktopSearchUI extends Widget {
	constructor(options) {
		...
		this.root
			.querySelector(".refresh-search-from-page-button")
			.addEventListener("click", () => this.parent.refreshSearchFromPage());
		this.root
			.querySelector(".expand-manga-posts")
			.addEventListener("click", (e) => {
				this.parent.searchView.toggleExpandingMediaIdsByDefault();
			});

		this.root
			.querySelector(".refresh-search-button")
			.addEventListener("click", () => this.parent.refreshSearch());
		...
		this.root
			...
			.addEventListener("click", (e) => new SettingsDialog());
	...
	refreshUi() {
		...
		if (this.toggleLocalNavigationButton) {
			this.toggleLocalNavigationButton.hidden =
				this.parent._localNavigationTree == null ||
				!this.parent.canShowLocalNavigation;
			this.toggleLocalNavigationButton.querySelector(".font-icon").innerText =
				this.parent._localNavigationVisible
	...
	refreshExpandMangaPostsButton() {
		...
		button.dataset.popup = enabled
			? "Collapse manga posts"
			: "Expand manga posts";
		button.querySelector(".font-icon").innerText = enabled
			? "close_fullscreen"
			: "open_in_full";
	...
	refreshRefreshSearchFromPage() {
		...
		this.root.querySelector(".refresh-search-from-page-button").dataset.popup =
			`Refresh search from page ${startPage}`;
	}
}
