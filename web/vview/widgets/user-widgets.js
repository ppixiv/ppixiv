...
export class GetUserInfo extends AsyncLookup {
	constructor({
		...
		ppixiv.userCache.addEventListener(
			"usermodified",
			(e) => {
				if (e.userId == this._id) this.refresh();
			},
			this._signal,
		);
	}
...
export class GetUserIdFromMediaId extends AsyncLookup {
	...
	async _refreshInner() {
		...
		if (this._id != null) {
			...
			if (type == "user") this._info.userId = id;
			else {
...
export class AvatarWidget extends Widget {
	...
	onrefresh({ userId, userInfo }) {
		...
		this.img.src =
			cachedProfileUrl ?? userInfo?.imageBig ?? helpers.other.blankImage;
		...
		this.root.querySelector(".follow-icon").hidden = !(
...
class ImageCanvasFilter {
	...
	_updateCanvas = () => {
		...
		if (!this.img.complete) currentUrl = helpers.other.blankImage;

		if (currentUrl == this._currentUrl) return;
		...
		if (currentUrl == helpers.other.blankImage) return;
		...
		if (this.overlay) this.overlay(this.ctx, this.img);
...
class CreepyEyeWidget extends Widget {
	...
	onevent = (e) => {
		...
		const expandRatio = 1.5; // extend 200% more mouse hover distance

		const width = bounds.width;
		const height = bounds.height;

		this.hover =
			bounds.left - width * expandRatio <= e.clientX &&
			e.clientX <= bounds.right + width * expandRatio &&
			bounds.top - height * expandRatio <= e.clientY &&
			e.clientY <= bounds.bottom + height * expandRatio;

		if (e.type === "mouseover") this.hover = true;
		if (e.type === "mouseout") this.hover = false;
...
class FollowWidget extends Widget {
	constructor({
		...
		this.root
			.querySelector(".follow-button-public")
			.addEventListener("click", (e) => this._clickedFollow(false));
		this.root
			.querySelector(".follow-button-private")
			.addEventListener("click", (e) => this._clickedFollow(true));
		this.root
			.querySelector(".toggle-follow-button-public")
			.addEventListener("click", (e) => this._clickedFollow(false));
		this.root
			.querySelector(".toggle-follow-button-private")
			.addEventListener("click", (e) => this._clickedFollow(true));
		this.root
			.querySelector(".unfollow-button")
			.addEventListener("click", (e) => this._clickedUnfollow());
	...
	_userChanged = ({ userId }) => {
		if (!this.visible || userId != this.userId) return;
	...
	async loadUser() {
		...
		if (userInfo) this.data.userInfo = userInfo;
		else userInfo = ppixiv.userCache.getUserInfo(this.userId);
		...
		if (userProfile) this.data.userProfile = userProfile;
		else userProfile = ppixiv.userCache.getUserProfile(this.userId);
		...
		if (this.data.userInfo?.isFollowed) this._requestFollowInfo();

		// If we had to request the user info or profile, wait for them to complete and refresh again.
		if (userInfo) this.data.userInfo = await userInfo;
		if (userProfile) this.data.userProfile = await userProfile;
		...
		if (following != null) {
			if (following) {
				...
				if (followingPrivately != null) {
					this.root.querySelector(".toggle-follow-button-public").hidden =
						!followingPrivately;
					this.root.querySelector(".toggle-follow-button-private").hidden =
						followingPrivately;
				} else {
	...
	async _clickedMute() {
		...
		else await ppixiv.muting.addMute(this.userId, null, { type: "user" });
	}
	...
	_getInfoLinksForUser({ userInfo, userProfile, boothUrl } = {}) {
		...
		for (let link of div.querySelectorAll("a")) {
			...
			if (imageType == "booth") label = "Booth";
			else if (imageType == "fanbox") label = "Fanbox";
			else if (label.startsWith("www.")) label = label.substr(4);
		...
		for (let pickup of pickups) {
			if (pickup.type != "fanbox") continue;
		...
		if (boothUrl) extraLinks.push({ url: boothUrl, label: "Booth" });
	...
	_filterLinks(extraLinks) {
		...
		if (
			...
			filteredLinks[filteredLinks.length - 1].type == "separator"
		)
	...
	_findLinkImageType(url) {
		...
		if (
			...
			url.pathname.startsWith("/fanbox/")
		)
			return "fanbox";
		...
		for (let alt in altIcons) {
			...
			for (let domain of altIcons[alt]) {
				if (url.hostname == domain) return alt;

				if (url.hostname.endsWith("." + domain)) return alt;
			}
		}
		return null;
	}
...
class FollowTagWidget extends Widget {
	constructor({ userId, ...options }) {
		...
		ppixiv.userCache.addEventListener(
			"usermodified",
			({ userId }) => {
				if (userId == this.userId) this.load();
			},
			this._signal,
		);
	}
