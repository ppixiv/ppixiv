...
import {
	MenuOptionButton,
	MenuOptionRow,
	MenuOptionOptionsSetting,
	MenuOptionsThumbnailSizeSlider,
} from "/vview/widgets/menu-option.js";
import {
	MenuOptionSliderSetting,
	MenuOptionToggleSetting,
} from "/vview/widgets/menu-option.js";
	...
	return {
		...
		noHideCursor: () => {
			return new MenuOptionToggleSetting({
				...
				explanationDisabled:
					"Don't hide the cursor while the mouse isn't moving.",
			});
		},
		...
		bookmarkPrivatelyByDefault: () => {
			return new MenuOptionToggleSetting({
				...
				explanationDisabled: ppixiv.mobile
					? null
					: "Pressing Ctrl-B will bookmark publically",
				explanationEnabled: ppixiv.mobile
					? null
					: "Pressing Ctrl-B will bookmark privately",
			});
		},
		...
		quickView: () => {
			return new MenuOptionToggleSetting({
				...
				setting: "quick_view",
				explanationEnabled:
					"Navigate to images immediately when the mouse button is pressed",

				check: () => {
					// Only enable changing this option when using a mouse.  It has no effect
					// on touchpads.
					if (PointerListener.pointerType === "mouse") return true;
		...
		autoPan: () => {
			return new MenuOptionToggleSetting({
				...
				setting: "auto_pan",
				explanationEnabled:
					"Pan images while viewing them (drag the image to stop)",
			});
		},
...
export class SettingsDialog extends DialogWidget {
	...
	addPages() {
		...
		if (ppixiv.sendImage.enabled) this._createPageButton("linkedTabs");
	...
	_createPageButton(name) {
		...
		if (this.phone) helpers.html.setClass(pageButton, "selected", true);
	...
	showPage(settingsPage) {
		...
		if (this._visiblePageName == settingsPage) return;

		// Remove the widget page or dialog if it still exists.
		if (this._pageWidget != null) this._pageWidget.shutdown();
		...
		if (!this.phone) this.header = pageTitles[settingsPage];
	...
	static _fillPage({ settingsPage, pageWidget, pageContainer }) {
		...
		let isSettingsList =
			settingsPage != "tagMuting" && settingsPage != "userMuting";
		if (isSettingsList) pageContainer.classList.add("settings-list");
		...
		let pages = {
			thumbnail: () => {
				...
				if (!ppixiv.native) settingsWidgets.expandMangaPosts();
			},
			image: () => {
				...
				if (!ppixiv.native)
					// native mode doesn't support manga pages
					settingsWidgets.slideshowSkipsManga();

				if (ppixiv.mobile) settingsWidgets.displayMode();
			...
			other: () => {
				...
				if (!ppixiv.mobile) settingsWidgets.limitSlideshowFramerate();

				if (!ppixiv.native) settingsWidgets.preloadManga();

				if (!ppixiv.native && ppixiv.mobile) settingsWidgets.openPixiv();
				...
				// a PWA.
				if (ppixiv.native || !ppixiv.mobile) settingsWidgets.stageSlideshow();

				if (!ppixiv.native) settingsWidgets.showAiIcon();

				if (!ppixiv.native || !ppixiv.mobile) settingsWidgets.touchpadMode();
			},
