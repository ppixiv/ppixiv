...
export class SettingsDialog extends DialogWidget {
  ...
  addPages() {
    ...
    if (ppixiv.imageTranslations.supported) this._createPageButton("translation");
  ...
  static _fillPage({ settingsPage, pageWidget, pageContainer }) {
    ...
    let pages = {
      ...
      other: () => {
        ...
        if (!ppixiv.native && !ppixiv.mobile) settingsWidgets.disabledByDefault();

        if (!ppixiv.mobile) {
          // Firefox's contextmenu behavior is broken, so hide this option.
          if (navigator.userAgent.indexOf("Firefox/") == -1) settingsWidgets.invertPopupHotkey();
        ...
        if (!ppixiv.native && window.showOpenFilePicker != null) settingsWidgets.importExtraData();
...
export class SlideshowStagingDialog extends DialogWidget {
  ...
  visibilityChanged() {
    ...
    if (!this.visible) {
      ...
      if (helpers.args.location.toString() == this.url.toString()) ppixiv.phistory.back();
    }
  }
}
