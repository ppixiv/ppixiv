...
export default class Settings extends EventTarget {
  ...
  configure(key, { sticky = false, session = false, defaultValue = null }) {
    if (sticky) {
      ...
      if (this.stickySettings[key] === undefined) this.stickySettings[key] = null;
    }

    if (session) this.sessionSettings[key] = true;

    if (defaultValue != null) this.defaults[key] = defaultValue;
  }

  _getStorageForKey(key) {
    if (this.sessionSettings[key]) return sessionStorage;
    else return localStorage;
  }

  // Wait until we return to the event loop, then clear any cached settings.
  async _queueClearCache() {
    if (this._clearCacheQueued || Object.keys(this.cache).length == 0) return;
  ...
  _getFromStorage(key, defaultValue) {
    ...
    if (key in this.cache) {
      ...
      if (value === this.noValue) return defaultValue;
      else return value;
    }
  ...
  get(key, defaultValue) {
    if (key in this.defaults) defaultValue = this.defaults[key];

    // If this is a sticky setting and we've already read it, use our loaded value.
    if (this.stickySettings[key]) return this.stickySettings[key];
    ...
    if (this.stickySettings[key] !== undefined) this.stickySettings[key] = result;
  ...
  set(key, value) {
    ...
    if (value === undefined) throw "Key can't be set to undefined: " + key;

    // If this is a sticky setting, replace its value.
    if (this.stickySettings[key] !== undefined) this.stickySettings[key] = value;
  ...
  adjustZoom(setting, down) {
    ...
    if (typeof value != "number" || isNaN(value)) value = 4;
