...
export default class Actions {
  ...
  static async _bookmarkAddInternal(mediaId, options) {
    ...
    if (newBookmarkId == null) newBookmarkId = mediaInfo.bookmarkData ? mediaInfo.bookmarkData.id : null;
    ...
    ppixiv.message.show(wasBookmarked ? "Bookmark edited" : options.private ? "Bookmarked privately" : "Bookmarked");
  }
  ...
  static async bookmarkAdd(mediaId, options) {
    if (helpers.mediaId.isLocal(mediaId)) return await this._localBookmarkAdd(mediaId, options);

    if (options == null) options = {};

    // If bookmark_privately_by_default is enabled and private wasn't specified
    // explicitly, set it to true.
    if (options.private == null && ppixiv.settings.get("bookmark_privately_by_default")) options.private = true;
    ...
    if (mediaInfo.bookmarkData == null) {
      ...
      if (options.tags != null) RecentBookmarkTags.updateRecentBookmarkTags(options.tags);
  ...
  static async bookmarkRemove(mediaId) {
    if (helpers.mediaId.isLocal(mediaId)) return await this._localBookmarkRemove(mediaId);
  ...
  static async _localBookmarkAdd(mediaId, options) {
    ...
    ppixiv.message.show(wasBookmarked ? "Bookmark edited" : type == "folder" ? "Bookmarked folder" : "Bookmarked");
  }
  ...
  static async addNewBookmarkTag(mediaId) {
    ...
    // Edit the bookmark.
    if (helpers.mediaId.isLocal(mediaId)) await Actions._localBookmarkAdd(mediaId, { tags: activeTags });
  ...
  static async follow(userId, followPrivately, { tag = null } = {}) {
    ...
    // If bookmark_privately_by_default is enabled and private wasn't specified
    // explicitly, set it to true.
    if (followPrivately == null && ppixiv.settings.get("bookmark_privately_by_default")) followPrivately = true;
  ...
  static async downloadIllust(mediaId, downloadType) {
    ...
    let pages = [];
    for (let page = 0; page < mediaInfo.mangaPages.length; ++page) pages.push(page);
    ...
    try {
      for (let page of pages) {
        ...
        if (translatedCanvas != null) {
          let blob = await helpers.other.canvasToBlob(translatedCanvas, { type: "image/jpeg", quality: 1 });
  ...
  static generateFilename(mediaInfo, userInfo, ext, mangaPage, isMangaPage) {
    if (ppixiv.settings.get("use_original_download_filename")) {
      // {pid}_p{page_number}.{ext}
      let filename = mediaInfo.illustId + "_p0";
      if (isMangaPage) filename = mediaInfo.illustId + "_p" + mangaPage;
      return filename + "." + ext;
    } else {
  ...
  static isDownloadTypeAvailable(downloadType, mediaInfo) {
    ...
    if (downloadType == "ZIP") return mediaInfo.illustType != 2 && mediaInfo.pageCount > 1;
